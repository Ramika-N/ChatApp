...
export default function ContactScreen() {
    ...
    const [countryCode, setCountryCode] = useState<CountryCode>("LK"); // default county code
                ...
                <View className="p-5 items-center">
                    ...
                    <View className="mt-5 w-full">
                        ...
                        <View className='mt-2  flex flex-row justify-center'>
                            <TextInput inputMode='tel' className='h-16 font-bold text-lg border-y-2 border-y-green-600 w-[18%] ' placeholder='+94' value={country? `+${country.callingCode}`:callingCode} editable={false}
                                onChangeText={(text) => {
                                    setCallingCode(text);
                                }}
                            />
                            <TextInput inputMode='tel' className='h-16 font-bold text-lg border-y-2 border-y-green-600 w-[80%] ml-2 ' placeholder='77 ### ####' value={phoneNo}
                                onChangeText={(text) => {
                                    setPhoneNo(text);
                                }}
                            />
